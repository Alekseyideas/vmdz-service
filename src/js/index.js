...
(function planPage() {
    ...
    setListInModal() {
      ...
      this.exersises.map((ex) => {
        ...
        <div class="form-check">
          ...
          <a href="${ex.link}" class="form-check-label" for="${this.modalId}-${ex.id}">
            ${ex.title}
          </a>
        </div>
        `;
    ...
    setDataInBlock(data = []) {
      ...
      newArr.forEach((itm, i) => {
        ...
        <div class="d-flex ex-list-btns-wrapper">
          ...
          <button class="btn btn-primary btn-sm me-2 ${BTN_UP_EX}"  id="${BTN_UP_EX}-${this.blockId}-${itm.id}" data-id="${itm.id}" type="button" ${isDisabledUp}>${UI_ARROW}</button>
          <button class="btn btn-primary btn-sm ${BTN_DOWN_EX}" id="${BTN_DOWN_EX}-${this.blockId}-${itm.id}" data-id="${itm.id}" type="button" ${isDisabledDown}>${UI_ARROW}</button>
        </div>
      </li>
        `);
      });
  ...
  let practicalEx = null;

  if (btnAddPrExesises) {
    practicalEx = new Exercise(
      'practical-exesises',
      prExesisesModalId,
      'data/equipment-exesises.json'
    );
  ...
  if (selectProppose && textAreaProppose) {
    selectProppose.addEventListener('change', (e) => {
      if (e.target.value === 'another') {
        ...
        if (!textAreaProppose.classList.contains('d-none'))
          textAreaProppose.classList.add('d-none');
        inputValues.propose = e.target.value;
      }
    });
    textAreaProppose.addEventListener('change', (e) => {
      inputValues.propose = e.target.value;
    });
  }
  ...
  class CustomForm {
    ...
    inputsHandler() {
      ...
      titleInput.addEventListener('change', (e) => (this.title = e.target.value));
      bodyInput.addEventListener('change', (e) => (this.body = e.target.value));
    }
  ...
  function InputObj(id, defaultValue = '', select = false) {
    this.id = id;
  ...
  InputObj.prototype = {
    inputHandle: function () {
      const fnCl = this.isSelect ? 'change' : 'keyup';
      this.input.addEventListener(fnCl, (e) => {
        globalExersises[this.id] = e.target.value;
      });
    },
    clean: function () {
      this.input.value = '';
    ...
    init: function () {
      ...
      this.inputHandle();
      globalExersises[this.id] = this.value;
    },
  };
  ...
  new InputObj(THEME).init();
  new InputObj(GOAL).init();
  new InputObj(EQ).init();
  new InputObj(VID, '', true).init();

  const saveConspect = document.getElementById('saveConspect');

  if (saveConspect) {
    saveConspect.addEventListener('click', () => {
      if (
        !globalExersises[THEME] ||
        !globalExersises[GOAL] ||
        !globalExersises[EQ] ||
        !globalExersises[VID]
      ) {
        console.log('Вывод ошиюки если поля пустые');
        return null;
      }
      console.log('Ajax работа для обработки формы', globalExersises);
    });
  }
})();
//
...
(function pageGames() {
  ...
  if (accordion) {
    ...
    accordionFn();
    links.forEach((a) => {
      a.addEventListener('click', accordionFn);
    });

    function accordionFn() {
      if (!id && !(this && this.dataset && this.dataset.id)) return null;
      const vpId = (this && this.dataset && this.dataset.id) || id;
      const oldBtn = accordion.querySelector(`a[data-id='${cachCard}']`);
      const btn = accordion.querySelector(`a[data-id='${vpId}']`);
      if (vpId !== cachCard && oldBtn) oldBtn.classList.remove('active');
      if (btn) btn.classList.add('active');

      const oldCard = document.getElementById('vp-' + cachCard);
      const activeCard = document.getElementById('vp-' + vpId);
